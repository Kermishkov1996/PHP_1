...
function getUser($login, $pass) {
    ...
    if ($user = query("SELECT * FROM customers WHERE login = '$login' AND password = '$pass'", 1, IMAGES_DB)) {
...
function regUser($data) {
    ...
    if (query("SELECT * FROM customers WHERE login = '$login'", 1, IMAGES_DB)) {
        $res = false;
    } else {
        //если такого нет, то вносим в БД
        query("INSERT INTO customers (login, password, name, phone, e_mail) VALUES ('$login', '$pass', '{$data['new_name']}', {$data['new_phone']}, '{$data['new_mail']}')");
        $res = true;
    }
